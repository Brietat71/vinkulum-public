...
import numpy as np
out=Path('/tmp/vinkulum-sonde-inertie-binaire-2026-v1');out.mkdir(exist_ok=False)
...
for name in ('inertie_complement_binaire.py','intervalles_binary64.py','inertie_complement_dirigee.py'):shutil.copy2(ci/name,out/name)
...
for n in (32,128,512):
    ...
    for kind,p in [('naturel',None),('separateurs',perm)]:
        ...
        try:r.update(statut='certifie',certificat=certifier_inertie_binaire(di,mi,b,(2*np.pi*80)**2,permutation=p))
