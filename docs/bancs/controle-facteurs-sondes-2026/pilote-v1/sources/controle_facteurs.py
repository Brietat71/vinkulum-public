"""Contrôle expérimental : normes d'extension via facteur énergétique commun.
...
class ImageFaibleRang:
    ...
    def __init__(self, image, gauche, droite):
        ...
        self.ec = np.linalg.norm(defaut+calcul,axis=0)+self.gamma*np.linalg.norm(self.image,axis=0)
    ...
    def normes(self, v):
        ...
        return self.kappa*(np.linalg.norm(action,axis=0)+np.linalg.norm(calcul,axis=0))+self.ec@av


class ControleFacteurs(ControleComplement):
    """Comprime seulement l'image énergétique utilisée par les normes 2."""
    def __init__(self, reduction, profondeur=8, *, extension="gram"):
        ...
        if extension == "qr":
            ...
            ec = np.linalg.norm(np.abs(defaut)+erreur_qr, axis=0)
            ...
            self._h0, self._hw = np.linalg.norm(h0, axis=0), np.linalg.norm(h1, axis=0)
            ...
            self.facteurs = dict(dimension_image=q.shape[1],
    ...
    def _norme_extension_d(self, y, dx=None, gram=None):
        if self.extension == "gram":
            if gram is None:
                if dx is None:
                    ...
                    dx = r.d@x
                gram = dx.T@dx
            trace = float(np.sum(np.maximum(np.diag(gram),0.)))
        ...
        eps_qr = np.linalg.norm(self._ec0+self._ecw@ay)
        eps_formation = np.linalg.norm(self._h0+self._hw@ay)
        return float(self._kappa_d*(norme(petit)+np.linalg.norm(calcul))
