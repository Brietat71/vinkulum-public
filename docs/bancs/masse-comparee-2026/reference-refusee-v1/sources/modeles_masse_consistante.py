...
from confronte_ports_exudyn import lire,sparse_pack,juger
...
def generer(source,destination):
    ...
    for n in (32,128,512):
        ...
        arrays=list(map(np.asarray,fields));judgement=juger(d,m,metric,*arrays)
        ...
        info=dict(precision=[70,90],ecart_relatif_max=error,maxima=judgement['maxima'],
            ...
            certification_machine=False,compteurs=[x.compteurs for x in refs])
