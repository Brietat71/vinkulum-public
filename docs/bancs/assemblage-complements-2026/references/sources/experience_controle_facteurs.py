"""Campagne 40 Hz : contrôle par facteurs, contrôle initial, LU et HCB.
...
FACTEURS = ("facteurs_controle",)
CANDIDATS = FACTEURS + ("inertie_controle",)
INERTIE = CANDIDATS
CONTROLES = CANDIDATS
SCHEMA = 3
TYPE_CAMPAGNE = "controle_facteurs_60_essais"
EXTENSION_FACTEURS = "gram"
GAMMA_INERTIE = float((2*math.pi*80)**2)
OMEGA_MAX = float(2*math.pi*40)
BUDGETS_INERTIE = dict(budget_operations=100_000_000,
...
SOURCES = ("experience_controle_facteurs.py", "controle_facteurs.py",
           "experience_inertie_contrainte.py", "experience_krylov_contraint.py",
           "inertie_complement_dirigee.py", "krylov_contraint.py",
...
def parametres_certificat(variante):
    """Paramètres figés, également conservés lorsqu'un certificat refuse."""
    if variante in INERTIE:
        return dict(type="inertie_kkt_dirigee", precision_decimal=32,
                    gamma_binary64=GAMMA_INERTIE, gamma_hex=GAMMA_INERTIE.hex(),
                    frequence_seuil_hz=80., permutation="naturelle",
                    **BUDGETS_INERTIE)
    return dict(type="aucun")


def parametres_reduction(variante):
    return dict(blocs=4, max_directions=128, directions_retenues=1,
                profondeur_controle=8 if variante in CONTROLES else None)


def parametres_controle(variante):
    if variante in FACTEURS:
        return dict(classe="ControleFacteurs", extension=EXTENSION_FACTEURS,
                    reparation="ImageFaibleRang", profondeur=8,
                    certification_machine=False)
    if variante in CONTROLES:
        return dict(classe="ControleComplement", profondeur=8,
                    certification_machine=False)
    return dict(type="aucun")
...
def worker_candidat(entree, reference, variante, n, dossier):
    ...
    with warnings.catch_warnings(record=True) as avertissements:
        ...
        try:
            ...
            # Appel d'inertie identique aux deux variantes et au lot précédent.
            # Seuil binary64 figé ; la preuve vise ces coefficients exacts.
...
def verifier_diagnostic_facteurs(essai):
    """Structure et paramètres des audits ; aucun recalcul des facteurs."""
    diagnostic = essai["controle_facteurs"]
    if (diagnostic["extension"] != EXTENSION_FACTEURS
            or diagnostic["certification_machine"] is not False):
        raise ValueError("formule de facteurs différente du protocole")
    for nom in ("gamma_gram", "gamma_sommes"):
        v = diagnostic[nom]
        if (not isinstance(v, (int, float)) or isinstance(v, bool)
                or not math.isfinite(v) or not 0 <= v < 1):
            raise ValueError("coefficient d'arrondi des facteurs invalide")
    images = diagnostic["reparation"]
    if set(images) != {"masse", "deformation"}:
        raise ValueError("images de réparation incomplètes")
    colonnes = essai["reduction"]["taille_complement_reduit"]
    for image in images.values():
        if (image["rang_facteur"] != 1 or image["colonnes_image"] != colonnes
                or image["certification_machine"] is not False
                or not isinstance(image["kappa"], (int, float))
                or isinstance(image["kappa"], bool)
                or not math.isfinite(image["kappa"]) or image["kappa"] < 1):
            raise ValueError("dimensions ou orthogonalité de l'image invalides")
        valeurs = image["defaut_colonnes"]
        if len(valeurs) != colonnes or not all(
                isinstance(v, (int, float)) and not isinstance(v, bool)
                and math.isfinite(v) and v >= 0 for v in valeurs):
            raise ValueError("défauts de l'image invalides")
...
def analyser(rapport):
    ...
    for entree in rapport["essais"]:
        ...
        if e["variante"] in CANDIDATS:
            ...
            if e["statut"] == "termine":
                ...
                if (cert["precision_decimal"] != parametres["precision_decimal"]
                        or cert["certification_machine"] is not True
                ...
                if e["variante"] in INERTIE:
                    if (cert["lambda_min"] != GAMMA_INERTIE
                            ...
                            or cert["permutation_physique"] != list(range(dimension))):
    ...
    for n in MAILLAGES:
        for v in VARIANTES:
            ...
            lignes.append(ligne)
    return dict(cible=CIBLE, nombre_essais=60, type_campagne=TYPE_CAMPAGNE, configurations=lignes,
...
def campagne(dossier, entrees, py_candidat, py_lu, py_hcb, historique):
    ...
    rapport = dict(schema=SCHEMA, type_campagne=TYPE_CAMPAGNE, date_utc=maintenant(), statut="en_cours", cpu=8,
        ...
        protocole=dict(maillages=list(MAILLAGES), max_hz=40., frequences=257, charges=6,
            ...
            cout="QR, sélection eigsh, B=fl(M Phi), certificat d'inertie frais, Krylov, contrôle frais et ses facteurs éventuels, champs et normes M/D",
    ...
    if pythons["candidat"] != pythons["lu"]:
        raise ValueError("candidat et LU doivent employer le même Python gelé")
...
def main():
    ...
    if args.worker:
        ...
        print("Intégrité et qualifications de campagne conformes : "+str(args.verifier))
