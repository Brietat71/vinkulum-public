...
def references(entree, sortie):
    ...
    for w in omega:
        ...
        dm, dd = m@(x-y), d@(x-y)
        em = np.sqrt(np.maximum(np.sum((x-y)*dm, axis=0), 0.))
        ed = np.linalg.norm(dd, axis=0)
        nm = np.sqrt(np.sum(y*(m@y), axis=0))
        nd = np.linalg.norm(d@y, axis=0)
        ecarts.append(float(max(np.max(em/nm), np.max(ed/nd))))
        champs.append(y)
    if max(ecarts) > CIBLE/100:
    ...
    return dict(precision=[70, 90], ecart_relatif_max=max(ecarts),
            ...
            for j, w in enumerate(omega):
                if variante == "vinkulum_api":
                    ...
                    if variante == "vinkulum_reduit":
                        ...
                        x = reponse(r, w, forces, resolution="spectrale")
                    else:
...
def campagne(dossier, python_vinkulum, python_exudyn, cpu):
    ...
    rapport = dict(schema=1, date_utc=datetime.now(timezone.utc).isoformat(),
        ...
        protocole=dict(repetitions=3, echauffements=1, frequences=257, charges=6,
            ...
            diagnostic_hcb_comptabilise=True,
