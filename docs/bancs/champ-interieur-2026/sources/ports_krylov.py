...
class InterieurKrylov:
    ...
    def __init__(self, k, m, b, lambda_min, omega_max, tolerance=1e-6,
                 ...
                 facteur_energie=None, seuil_semence=0.):
        ...
        self.facteur_energie = facteur_energie
        valeurs = [self.lambda_min, self.omega_max, self.tolerance, seuil_dependance, seuil_semence]
        if (not np.all(np.isfinite(valeurs)) or self.lambda_min <= 0
                or self.omega_max < 0 or self.tolerance <= 0
                or not 0 < seuil_dependance < 1 or seuil_semence < 0):
        ...
        self._seuil_dependance = seuil_dependance
        self._max_directions = max_directions
        self._dernier_bloc = self.v.copy()
        candidat = self._resoudre(self.b)
        if seuil_semence:
            normes = np.sqrt(np.maximum(np.diag(self._gram_k(candidat, candidat)), 0.))
            candidat = candidat[:, normes > seuil_semence]
        for q in range(1, max_blocs+1):
            ...
            if nouveau.shape[1] == 0:
                if not self.v.shape[1]:
                    # Espace vide : B nul ou semences écartées ; le résidu
                    # conserve la charge entière, sans succès implicite.
                    self._preparer(q)
                self.statut = ("tolerance_estimee" if self.borne_uniforme <= self.tolerance
                               else "stagnation")
                break
            self.v = np.column_stack((self.v, nouveau))
            self._dernier_bloc = nouveau
            self._preparer(q)
    ...
    def enrichir(self):
        """Ajoute un bloc en réutilisant le facteur et la base déjà construits.

        L'appelant choisit son critère physique d'arrêt et son budget d'appels.
        Le budget de directions initial reste impératif. Une stagnation ne
        signifie jamais que ce nouveau critère est satisfait.
        """
        reste = self._max_directions-self.v.shape[1]
        if reste <= 0:
            self.statut = "budget_directions"
            return False
        candidat = self._resoudre(self.lambda_min*(self.m@self._dernier_bloc)
                                   if self.v.shape[1] else self.b)
        nouveau = self._orthogonaliser(candidat, self._seuil_dependance)[:, :reste]
        if nouveau.shape[1] == 0:
            self.statut = "stagnation"
            return False
        self.v = np.column_stack((self.v, nouveau))
        self._dernier_bloc = nouveau
        profondeur = len(self.historique)+1
        self._preparer(profondeur)
        self.taille_interieure = self.v.shape[1]
        self.historique.append(dict(blocs=profondeur, directions=self.v.shape[1],
                                    borne_uniforme=self.borne_uniforme,
                                    defaut_gram=self.defaut_gram))
        self.statut = ("tolerance_estimee" if self.borne_uniforme <= self.tolerance
                       else "enrichi")
    ...
    def _resoudre(self, rhs):
        ...
        if self.facteur_energie is not None:
            x = self.facteur_energie.solve(rhs)
            r = rhs-self._produit_k(x)
    ...
    def _produit_k(self, x):
        return self.k@x if self.facteur_energie is None else self.facteur_energie.produit(x)

    def _gram_k(self, x, y):
        return x.T@(self.k@y) if self.facteur_energie is None else self.facteur_energie.gram(x, y)

    def _orthogonaliser(self, candidat, seuil):
        w = candidat.copy()
        if self.v.shape[1]:
            g = _sym(self._gram_k(self.v, self.v))
            for _ in range(2):
                w -= self.v@solve(g, self._gram_k(self.v, w), assume_a="pos")
        gram = _sym(self._gram_k(w, w))
        normes = np.sqrt(np.maximum(np.diag(gram), 0.))
        initiales = np.sqrt(np.maximum(np.diag(self._gram_k(candidat, candidat)), 0.))
        actifs = normes > seuil*np.maximum(initiales, np.finfo(float).tiny)
        if not np.any(actifs):
            return w[:, :0]
        w = w[:, actifs]/normes[actifs]
        valeurs, vecteurs = eigh(_sym(self._gram_k(w, w)))
    ...
    def _preparer(self, profondeur):
        ...
        if r:
            ...
            self.w = self.v.copy()
        kw = self._produit_k(self.w)
        mw = self.lambda_min*(self.m@self.w)
        self.g = _sym(self._gram_k(self.w, self.w))
        ...
        self.f = mw-kw@self.t
        if self.facteur_energie is None:
            ke = self._resoudre(self.e)
            kf = self._resoudre(self.f) if r else self.f.copy()
            delta = _norme_gram(self.e.T@ke)
            dual_f = None
        else:
            dual_e = self.facteur_energie.dual(self.e)
            dual_f = self.facteur_energie.dual(self.f) if r else self.f.copy()
            delta = _norme_gram(dual_e.T@dual_e)
        p = np.eye(r)
    ...
    def borne_ponctuelle(self, omega):
        ...
        residu = self.b-self._produit_k(x)+mu*self.lambda_min*(self.m@x)
        if self.facteur_energie is None:
            gram = _sym(residu.T@self._resoudre(residu))
        else:
            dual = self.facteur_energie.dual(residu)
            gram = _sym(dual.T@dual)
        return gram/(1-mu)
