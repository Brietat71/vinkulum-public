"""Campagne 40 Hz : certificats inertie/trace, LU corrigée et HCB historique.

Les 84 processus comprennent une chauffe et trois mesures par variante et
...
INERTIE = ("inertie_champs", "inertie_controle")
TRACE = ("krylov_champs", "krylov_controle")
CANDIDATS = INERTIE + TRACE
CONTROLES = ("inertie_controle", "krylov_controle")
SCHEMA = 2
TYPE_CAMPAGNE = "inertie_contrainte_84_essais"
...
SOURCES = ("experience_inertie_contrainte.py", "experience_krylov_contraint.py",
...
def parametres_certificat(variante):
    ...
    if variante in INERTIE:
        return dict(type="inertie_kkt_dirigee", precision_decimal=32,
                    ...
                    **BUDGETS_INERTIE)
    if variante in TRACE:
        return dict(type="trace_inverse_selectionnee_dirigee", precision_decimal=16)
    ...
    with warnings.catch_warnings(record=True) as avertissements:
        ...
        try:
            ...
            if variante in INERTIE:
                ...
                # Appel identique au pilote historique de la campagne trace16.
...
def analyser(rapport):
    ...
    if len(rapport["essais"]) != 84:
        raise ValueError("campagne incomplète : 84 essais requis")
    ...
    for n in MAILLAGES:
        for v in VARIANTES:
            ...
            lignes.append(ligne)
    return dict(cible=CIBLE, nombre_essais=84, type_campagne=TYPE_CAMPAGNE, configurations=lignes,
