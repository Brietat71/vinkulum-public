...
def lire_matrice(a, n, colonnes=None):
    liste(a, n)
    ...
    for ligne in a:
        ...
        for entree in ligne:
            j, v = liste(entree, 2)
            if type(j) is not int or not dernier < j < nc:
...
def verifier_document(doc):
    ...
    if type(doc) is not dict or set(doc) != attendus or doc['schema'] not in (SCHEMA, 'vinkulum.lineaire.creux.1'):
