"""Certificat d’inertie lié à l’extension native, sans compilation à l’appel."""
import ctypes as ct
import hashlib
import math
import time
...
def certifier_inertie_compilee(d_i, mii, b, gamma, *, bibliotheque,
    ...
    d,m = matrice_entree(d_i,'D_i'),matrice_entree(mii,'M_ii')
    n = d.shape[1]
    ...
    if not isinstance(bibliotheque,BibliothequeInertie):
        raise TypeError('adaptateur natif requis')
    from vinkulum import _vinkulum
    status, pm, pk, sm, diagnostic, interval, phases = _vinkulum._certificat_inertie_binaire(
        n, s, (d.indptr.tolist(), d.indices.tolist(), d.data.tolist()),
        (m.indptr.tolist(), m.indices.tolist(), m.data.tolist()),
        b.ravel().tolist(), gamma, perm.tolist(), budget_operations,
        budget_coefficients, budget_rectangulaire)
    mass_pivots, kkt_pivots = (Pivot*n).from_buffer_copy(pm), (Pivot*(n+s)).from_buffer_copy(pk)
