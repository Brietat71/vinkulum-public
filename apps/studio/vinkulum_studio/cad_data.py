...
class CadGeometry:
    ...
    def __post_init__(self):
        if not isinstance(self.brep_mm, str) or not 1 <= len(self.brep_mm) <= 2_000_000:
            raise ValueError("BREP absent ou trop volumineux (2 Mo maximum).")
        if not self.brep_mm.isascii() or "CASCADE Topology" not in self.brep_mm[:200]:
            raise ValueError("BREP ASCII OCCT requis.")
        if (
            not isinstance(self.volume_m3, (int, float))
            or not math.isfinite(self.volume_m3)
            or self.volume_m3 <= 0
        ):
            raise ValueError("Volume CAD strictement positif requis.")
        if (
            ...
            raise ValueError("Tenseur CAD 3×3 requis.")
        ...
        for point in self.vertices_m:
            if (
                ...
                raise ValueError("Sommet CAD invalide.")
        if (
            not isinstance(self.triangles, tuple)
            or not 4 <= len(self.triangles) <= 30_000
        ):
            raise ValueError("Limite de triangles CAD dépassée.")
        for triangle in self.triangles:
            if (
                ...
                raise ValueError("Triangle CAD invalide.")
        if (
            ...
            raise ValueError("Historique CAD invalide.")
        for value in (self.occt_version, self.build123d_version):
            if not isinstance(value, str) or not 1 <= len(value) <= 64:
                raise ValueError("Version CAD absente.")
    ...
    def from_dict(cls, data):
        if not isinstance(data, dict) or set(data) != {f.name for f in fields(cls)}:
            raise ValueError("Champs CAD inconnus ou manquants.")
        data = dict(data)
