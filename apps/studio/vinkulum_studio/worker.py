...
from .model import (G, INERTIA, RHO, PARAMETER_UNITS, UNITS, Parameters,
...
def simulate(parameters, run_id):
    ...
    body = solver.corps("masse", p.mass, [INERTIA, 0., 0., 0., INERTIA, 0., 0., 0., INERTIA],
                        [x, 0., z])
    ...
    return {"schema_version": 1, "run_id": run_id, "parameters": asdict(p),
            "status": "completed", "samples": samples,
            "manifest": {"app_version": __version__, "kernel_version": vinkulum.__version__,
                         "kernel_sha256": hashlib.sha256(Path(native.__file__).read_bytes()).hexdigest(),
                         "python": platform.python_version(), "platform": platform.platform(),
                         "machine": platform.machine(), "completed_utc": datetime.now(timezone.utc).isoformat(),
                         "method": "generalized-alpha / SO(3)", "rho_infinity": RHO,
                         "gravity_m_s2": G, "body_inertia_kg_m2": INERTIA,
                         "initial_angular_velocity_rad_s": 0., "threads": 2,
                         "units": UNITS, "parameter_units": PARAMETER_UNITS,
...
def main():
    if len(sys.argv) != 3:
        raise SystemExit("Usage: python -m vinkulum_studio.worker input.json output.json")
    try:
        ...
        write_json(sys.argv[2], {"status": "failed", "error_type": type(exc).__name__,
