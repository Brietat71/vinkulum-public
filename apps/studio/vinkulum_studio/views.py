...
class PendulumView(QWidget):
    def __init__(self, parent=None):
        ...
        self.length = 1.
        self.sample = (0., .5, -math.sqrt(.75), math.pi / 6)
        self.setAccessibleName("Vue du pendule dans le plan x-z")
    ...
    def paintEvent(self, event):
        with QPainter(self) as painter:
            ...
            scale = min(self.width() * .39, (self.height() - 80) / 2) / self.length
            ...
            painter.setPen(QPen(QColor("#465569"), 6, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
            ...
            painter.drawText(18, 25, "PENDULE  /  PLAN X–Z")
...
class AngleView(QWidget):
    def __init__(self, parent=None):
        ...
        self.samples = ()
        self.index = 0
        self.setAccessibleName("Courbe de l'angle en fonction du temps")
    ...
    def paintEvent(self, event):
        with QPainter(self) as painter:
            ...
            if not self.samples:
                painter.drawText(rect, Qt.AlignmentFlag.AlignCenter, "Lancez le calcul pour afficher la trajectoire.")
                return
            duration = self.samples[-1][0]
            limit = max(1., max(abs(math.degrees(row[3])) for row in self.samples) * 1.12)
            def point(row):
                return QPointF(rect.left() + row[0] / duration * rect.width(),
                               rect.center().y() - math.degrees(row[3]) / limit * rect.height() / 2)
            ...
            for value in (-limit, 0., limit):
                ...
                painter.drawText(QRectF(0, y - 9, 47, 20), Qt.AlignmentFlag.AlignRight, f"{value:.1f}")
            ...
            painter.drawLine(QPointF(selected.x(), rect.top()), QPointF(selected.x(), rect.bottom()))
            painter.setBrush(QColor("#bd6425"))
            painter.drawEllipse(selected, 4, 4)
            painter.setPen(QColor("#25354b"))
            painter.drawText(QPointF(rect.left(), rect.bottom() + 23), "0")
            painter.drawText(QPointF(rect.right() - 105, rect.bottom() + 23), f"{duration:g} s  /  TEMPS")
