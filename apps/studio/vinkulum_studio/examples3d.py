...
def slider_crank():
    ...
    p = p.replace_object(
        replace(motor, name="Pivot moteur", motion=Law("lineaire", (0.0, 2.0)))
    )
...
def temporal_force():
    b = Body(new_id(), "Masse entraînée", dimensions=(0.2, 0.2, 0.2), mass=2.0)
    p = Project(
        new_id(),
        "Force croissante",
        bodies=(b,),
...
EXAMPLES = {
    "Pendule G0": pendulum,
    "Double pendule": double_pendulum,
    "Bielle-manivelle": slider_crank,
    "Force temporelle": temporal_force,
}
