"""Coercivité du complément par inertie creuse en intervalles binary64.
    ...
    def set(self, i, j, v):
        ...
        if v[0] == 0 == v[1]:
            if ancien:
                ...
                self.coefficients -= 1
            return
        if not all(math.isfinite(x) for x in v) or v[0] > v[1]:
            ...
            if choisi is None:
                raise InertieImpossible("pivot non séparé de zéro",
                    dict(pivot=k, intervalle=list(map(str, a)), pivots_effectues=len(pivots),
                         signature_partielle=signature, operations_binary64=ar.operations))
...
def certifier_inertie_binaire(d_i, mii, b, gamma, *,
    ...
    ar = IntervallesBinaires(budget_operations)
    ...
    if preuve["signature"] != [n, s, 0]:
        raise InertieImpossible("complément non coercif au seuil demandé",
            dict(gamma=gamma, signature=preuve["signature"], signature_attendue=[n, s, 0],
                 preuve=preuve, operations_binary64=ar.operations))
    fin = time.perf_counter()
    return dict(lambda_min=gamma, certification_machine=True, arithmetique="binary64_nextafter",
        encodage_intervalles="decimal_roundtrip_binary64",
        ...
        coefficients_initiaux=coefficients_initiaux, operations_binary64=ar.operations,
        coefficients_contraintes=b.size,
